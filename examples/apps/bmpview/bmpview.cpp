...
static const _TCHAR AppName[] = _T("BitmapView");
...
class _USERCLASS TBmpViewWindow : virtual public TWindow,
  ...
  protected:
    ...
    void      EvSize(uint sizeType, const TSize&);
...
class _USERCLASS TBmpViewApp : public TApplication {
  public:
  TBmpViewApp(LPCTSTR name) : TApplication(name) {};
  protected:
...
void TBmpViewApp::InitMainWindow() {
    TFrameWindow* frame = new TFrameWindow(0, GetName(), new TBmpViewWindow);
    frame->AssignMenu(GetName());
    frame->SetIcon(this, GetName());
    frame->SetAcceleratorTable(IDA_BMPVIEW);
    SetMainWindow(frame);
}
