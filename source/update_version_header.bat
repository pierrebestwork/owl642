...
for %%i in (SubWCRev.exe) do if not "%%~$PATH:i" == "" (
  ...
  if errorlevel 1 goto ErrorSubWCRevFailed
) else (
  if exist "%3" (
    echo %~nx0^(33^): warning: Could not find SubWCRev to update the version header with accurate build info.
    echo %~nx0^(34^): warning: The existing version header is left unchanged, and might contain inaccurate info.
    goto Finish
  ) else (
    echo %~nx0^(37^): warning: Could not find SubWCRev to create the version header with accurate build info. 
    echo %~nx0^(38^): warning: A version header setting the build number to 0 is generated.
...
echo %~nx0^(74^): error: SubWCRev failed to generate version header.
exit /B 1

:ErrorGenerationFailed

echo %~nx0^(79^): error: Failed to generate version header.
exit /B 2

:ErrorUpdateFailed

echo %~nx0^(84^): error: Failed to update version header %3.
exit /B 3
